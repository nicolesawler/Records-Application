...
private $f_size = F_SIZE;
    ...
    */ 
    ...
    public function getAllUploads($user_id)
    {
       try
       { 
          $stmt = $this->dbh->prepare("SELECT passport_id, name, original_name, mime_type, status, type, date_added "
...
//				) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8;");
			try{
				//$this->stmt->execute();
				echo "Not making table, but trying.";
			}
			catch(PDOException $e){
				array_push($this->error, $e->getMessage());
				return false;
   ...
   /**
    * @description MODIFIED BY NICOLE SAWLER
    * ORIGINAL BY icecub
    * Adds userid and type to db when uploading
    * @author nicole sawler
    * @param OROGINAL: $files (image), ADDED: user_id and type
    * @return obj
    */ 
...
public function uploadImages($files,$user_id,$type){
                                        ...
                                        if($this->check_img_mime($file['tmp_name'])){
                                                ...
                                                if($this->check_img_size($file['tmp_name'])){
                                                        ...
                                                        if (move_uploaded_file($file['tmp_name'], $uploadfile)) {
                                                                ...
                                                                $this->stmt = $this->dbh->prepare(""
                                                                        . "INSERT INTO ". DB_TABLE ." (user_id, name, original_name, mime_type, status, type) "
                                                                        . "VALUES (:user_id, :name, :oriname, :mime, 'ACTIVE', :type)");
	...
	/* Show the image in the browser */
        //Nicole: Added user id so that user can't view images that isn't theirs
	public function showImage($user_id,$id){
		$this->stmt = $this->dbh->prepare("SELECT name, original_name, mime_type FROM ". DB_TABLE ." WHERE passport_id=:id AND user_id = :user_id");
	...
	//Nicole: Added user id so that user can't download images that isn't theirs
	public function downloadImage($user_id,$id){
		$this->stmt = $this->dbh->prepare("SELECT name, original_name, mime_type FROM ". DB_TABLE ." WHERE passport_id=:id AND user_id = :user_id");
    ...
    /**
    * @description When user removes an image, change status to delete
    * @author nicole sawler
    * @param int
    * @return array
    */ 
   
    public function removeImage($user_id, $imgid)
    {
       try
       { 

          $stmt = $this->dbh->prepare("UPDATE `records`.`passport_images` SET `status`='DELETE' WHERE `passport_id`= :imgid AND user_id = :user_id");
           $stmt->bindparam(":user_id", $user_id);  
    ...
    // DONT USE THIS, USE FUNCTION ABOVE    
        
	/* Delete an image */
//	public function deleteImage($id){
//		$this->stmt = $this->dbh->prepare("SELECT name, original_name, FROM ". DB_TABLE ." WHERE passport_id=:id");
//
//		$this->bind(':id', $id);
//
//		try{
